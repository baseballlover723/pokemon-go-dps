...
function getChargeMoves(callback) {
  callback = callback || function (err, chrageMoves) {
    };
  ChargeMove.findAll({order: [['id', 'ASC']]}).then(function (chrageMoves) {
    callback(null, chrageMoves);
  }, function (err) {
    callback(err);
  });
}
...
function getPokemons(callback) {
  ...
  Pokemon.findAll({include: [{model: FastMove, as: 'fastMoves', attributes: ['id']}, {model: ChargeMove, as: 'chargeMoves', attributes: ['id']}], order: [['id', 'ASC']]}).then(function (pokemons) {
